//Aryan Sidana
//400242302
// LSD = 2 (J), thus bus speed = 48 MHz
// 2LSD = 0 (H), thus Distance Status LED = PN1 (LED D1)
#include <stdint.h>
...
void spin(uint32_t step,uint32_t t);
void spincounter(uint32_t step,uint32_t t);
...
//  status = VL53L1X_SetInterMeasurementInMs(dev, 200); /* in ms, IM must be > = TB */
  ...
  status = VL53L1X_StartRanging(dev);   /* This function has to be called to enable the ranging */
	...
	//Infinte loop to take as many measurments planes as needed
	while(1){
	if(GPIO_PORTM_DATA_R==0b00000001){ // Check for button press
		FlashLED1(1); //Flash PN1 every time a measument is taken
		
	//status = VL53L1X_StartRanging(dev);

	
	for(int i = 0; i < 64; i++) { //Repeat loop for 64 measurments
     ...
     // FlashLED1(1);
      ...
      sprintf(printf_buffer,"%u\r\n", Distance);
			...
			//Ouput distance using UART to PC
      UART_printf(printf_buffer);
		
	  
			spin(8,2); // Rotate motor 5.625 degrees with a 2ms step delay
  }
	
	spincounter(512,2); // Rotate motor 360 degrees with a 2ms step delay
